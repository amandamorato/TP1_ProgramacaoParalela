...
int main(int argc, char *argv[]) {
    ...
    int numElements_global = 1440;
    int numElements_local = 1440/size_of_cluster;
    int nums_global[numElements_global];
    int nums_local[numElements_local];
    ...
    if (process_rank == MESTRE){
        ...
        for (int i = 0; i < numElements_global; i++) {
            fscanf(f_in, "%d", &nums_global[i]);
        }
        
    }
    t0 = MPI_Wtime();
    MPI_Scatter(nums_global, numElements_local, MPI_INT, &nums_local, numElements_local, MPI_INT, MESTRE, MPI_COMM_WORLD);

    int divisores_local[numElements_local];
    for (int i = 0; i < numElements_local; i++){
        divisores_local[i] = qtd_Divisores_dinamico(nums_local[i]);
    }

    MPI_Gather(&divisores_local, numElements_local, MPI_INT, nums_global, numElements_local, MPI_INT, MESTRE, MPI_COMM_WORLD);    
    t1 = MPI_Wtime();
